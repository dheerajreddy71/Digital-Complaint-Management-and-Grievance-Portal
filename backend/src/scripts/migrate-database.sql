...
-- Temporarily change category to VARCHAR
ALTER TABLE complaints
...
-- Temporarily change status to VARCHAR
ALTER TABLE complaints
...
-- Temporarily change priority to VARCHAR
ALTER TABLE complaints
...
ALTER TABLE status_history
  MODIFY COLUMN from_status VARCHAR(50),
  MODIFY COLUMN to_status VARCHAR(50) NOT NULL;
...
UPDATE status_history SET from_status = 'Assigned' WHERE from_status NOT IN ('Open', 'In-progress', 'Resolved') AND from_status IS NOT NULL;
...
UPDATE status_history SET to_status = 'Assigned' WHERE to_status NOT IN ('Open', 'In-progress', 'Resolved');
...
UPDATE notifications SET type = 'Reminder' WHERE type = 'feedback_received';
UPDATE notifications SET type = 'System' WHERE type = 'system';
UPDATE notifications SET type = 'System' WHERE type NOT IN ('Assigned', 'StatusUpdate', 'Comment', 'SLA_Warning', 'Reminder');
