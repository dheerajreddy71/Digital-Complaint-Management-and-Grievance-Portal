...
async function fixAll() {
	...
	try {
		...
		await connection.query(
			"UPDATE users SET role = 'Citizen' WHERE role NOT IN ('Admin', 'Staff', 'Citizen')"
		);
		...
		const hasAssignedTo = columns.some(
			(col: any) => col.Field === "assigned_to"
		);
		...
		if (!hasAssignedTo) {
			...
			await connection.query(
				"ALTER TABLE complaints ADD COLUMN assigned_to INT"
			);
			await connection.query(
		...
		const hasResolvedAt = columns.some(
			(col: any) => col.Field === "resolved_at"
		);
		if (!hasResolvedAt) {
			await connection.query(
		...
		if (!hasIsOverdue) {
			await connection.query(
				"ALTER TABLE complaints ADD COLUMN is_overdue BOOLEAN DEFAULT FALSE"
			);
			console.log("   ✅ is_overdue added");
		}

		const hasIsRecurring = columns.some(
			(col: any) => col.Field === "is_recurring"
		);
		if (!hasIsRecurring) {
			await connection.query(
				"ALTER TABLE complaints ADD COLUMN is_recurring BOOLEAN DEFAULT FALSE"
			);
		...
		const [userColumns]: any = await connection.query(
			"SHOW COLUMNS FROM users"
		);

		const hasContactInfo = userColumns.some(
			(col: any) => col.Field === "contact_info"
		);
		if (!hasContactInfo) {
			await connection.query(
				"ALTER TABLE users ADD COLUMN contact_info VARCHAR(20)"
			);
			await connection.query(
				"UPDATE users SET contact_info = phone WHERE phone IS NOT NULL"
			);
			console.log("   ✅ contact_info added");
		}

		const hasAvailability = userColumns.some(
			(col: any) => col.Field === "availability_status"
		);
		if (!hasAvailability) {
			await connection.query(
				"ALTER TABLE users ADD COLUMN availability_status ENUM('available', 'busy', 'offline') DEFAULT 'available'"
			);
			console.log("   ✅ availability_status added");
		}

		const hasExpertise = userColumns.some(
			(col: any) => col.Field === "expertise"
		);
		if (!hasExpertise) {
			await connection.query(
				"ALTER TABLE users ADD COLUMN expertise VARCHAR(500)"
			);
		...
		const hasStaffId = feedbackColumns.some(
			(col: any) => col.Field === "staff_id"
		);
		if (!hasStaffId) {
		...
		const hasFeedbackUserId = feedbackColumns.some(
