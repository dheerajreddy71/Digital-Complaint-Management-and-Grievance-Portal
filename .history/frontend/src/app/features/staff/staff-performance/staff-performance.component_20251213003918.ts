					...
					<mat-card class="stat-card">
						...
						<div class="stat-content">
							<span class="stat-value">{{
								formatDuration(performance.average_resolution_hours || 0)
							}}</span>
...
export class StaffPerformanceComponent implements OnInit {
	...
	private loadRecentFeedback(staffId: string | number): void {
		this.analyticsService.getStaffFeedback(staffId).subscribe({
			next: (feedback) => {
				// Map Feedback to our display type with complaint_title
				this.recentFeedback = feedback.slice(0, 5).map((f) => ({
					rating: f.rating,
					comments: f.comments,
					created_at: f.created_at,
					complaint_title: `Complaint #${String(f.complaint_id).substring(0, 8)}`,
				}));
			},
		});
	}
