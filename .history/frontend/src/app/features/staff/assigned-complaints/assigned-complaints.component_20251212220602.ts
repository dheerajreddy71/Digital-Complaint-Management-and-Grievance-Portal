      ...
      >
        <mat-tab>
          ...
          <div class="tab-content">
            ...
            <div class="complaints-grid" *ngIf="!isLoading && activeComplaints.length > 0">
              <mat-card
              ...
              >
                <div class="card-header">
                  <div class="badges">
                    ...
                    <app-priority-badge [priority]="complaint.priority"></app-priority-badge>
                  </div>
                  <app-sla-indicator
                ...
                <div class="card-meta">
                  <span><mat-icon>category</mat-icon> {{ complaint.category | categoryLabel }}</span>
                ...
                <div class="card-actions">
                  <button
                    mat-stroked-button
                    color="primary"
                    (click)="updateStatus(complaint, 'resolved'); $event.stopPropagation()"
                  >
        ...
        <mat-tab>
          ...
          <div class="tab-content">
            <app-loading-spinner *ngIf="isLoadingResolved"></app-loading-spinner>
            ...
            <div class="complaints-grid" *ngIf="!isLoadingResolved && resolvedComplaints.length > 0">
              <mat-card
              ...
              >
                <div class="card-header">
                  <app-status-badge [status]="complaint.status"></app-status-badge>
                  <span class="resolved-date">{{ complaint.resolved_at | date: 'shortDate' }}</span>
                </div>
...
export class AssignedComplaintsComponent implements OnInit {
  ...
  private loadStaffStatus(): void {
    ...
    if (user) {
      this.isAvailable = user.is_available ?? true;
    }
  }
  ...
  loadActiveComplaints(): void {
    ...
    this.complaintService
      ...
      .subscribe({
        next: (response) => {
          this.activeComplaints = response.complaints;
          this.inProgressCount = this.activeComplaints.filter(
            (c) => c.status === ComplaintStatus.IN_PROGRESS
          ).length;
          this.overdueCount = this.activeComplaints.filter((c) => c.is_overdue).length;
    ...
    this.complaintService
      ...
      .subscribe({
        next: (response) => {
          this.resolvedTodayCount = response.total;
        },
      });
  }
  ...
  loadResolvedComplaints(): void {
    ...
    this.complaintService
      .getComplaints({
        assigned_to: staffId,
        status: `${ComplaintStatus.RESOLVED},${ComplaintStatus.CLOSED}`,
        limit: 50,
      })
      .subscribe({
        next: (response) => {
          this.resolvedComplaints = response.complaints;
  ...
  toggleAvailability(available: boolean): void {
    this.userService.updateAvailability(available).subscribe({
      next: () => {
        this.isAvailable = available;
        this.snackbar.success(
          available ? 'You are now available for assignments' : 'You are now unavailable'
        );
      },
    });
  }
  ...
  updateStatus(complaint: Complaint, status: string): void {
    this.complaintService.updateComplaint(complaint.id, { status }).subscribe({
      next: () => {
        this.activeComplaints = this.activeComplaints.filter((c) => c.id !== complaint.id);
