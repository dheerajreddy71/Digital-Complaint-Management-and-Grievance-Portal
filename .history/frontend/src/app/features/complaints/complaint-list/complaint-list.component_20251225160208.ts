		...
		<div class="page-container fade-in">
			<div class="page-header">
				<div class="header-title">
					...
					<p
						class="subtitle"
						*ngIf="authService.currentUser?.role === UserRole.STAFF"
					>
						Department visibility – view only
					</p>
				</div>
				<button
					mat-flat-button
				...
				<div class="complaints-grid" *ngIf="complaints.length > 0">
					...
					>
						...
						<mat-card-content>
							...
							<!-- Unassigned indicator for staff view -->
							<div
								class="unassigned-indicator"
								*ngIf="
							...
							>
								...
								<span>Unassigned – pick up from Work Queue</span>
							</div>
						</mat-card-content>
					</mat-card>
				</div>
...
export class ComplaintListComponent implements OnInit {
	...
	pickUpComplaint(event: Event, complaintId: string): void {
		...
		this.complaintService.assignComplaint(complaintId, staffId).subscribe({
			next: () => {
				this.snackbar.success("Complaint picked up successfully!");
				this.loadComplaints();
				this.isPickingUp = null;
			},
			error: (err) => {
				const errorMsg = err.error?.error || "Failed to pick up complaint";
