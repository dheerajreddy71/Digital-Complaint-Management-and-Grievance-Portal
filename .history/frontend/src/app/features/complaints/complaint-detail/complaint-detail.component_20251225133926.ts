...
@Component({
					...
					<div class="header-actions" *ngIf="isStaffOrAdmin || isOwner">
						...
						<mat-menu #statusMenu="matMenu">
							...
							<button
								mat-stroked-button
								color="accent"
								*ngIf="canReopenComplaint"
								(click)="reopenComplaint()"
							>
				...
				<div class="complaint-body">
					...
					<div class="sidebar">
						...
						<mat-card class="sidebar-card fade-in">
							...
							<mat-card-content>
								<div class="assignment-status" *ngIf="complaint.assigned_staff_name">
									<div class="assigned-badge">
										<mat-icon>check_circle</mat-icon>
										Assigned
									</div>
									<div class="staff-info">
										<div class="staff-name">{{ complaint.assigned_staff_name }}</div>
										<div class="staff-department" *ngIf="complaint.assigned_staff_department">
											{{ complaint.assigned_staff_department }}
										</div>
									</div>
								</div>
								<div class="assignment-status unassigned" *ngIf="!complaint.assigned_staff_name">
									<div class="unassigned-badge">
										<mat-icon>pending</mat-icon>
										Unassigned
									</div>
									<p class="unassigned-text">Waiting for staff pickup</p>
								</div>

								<mat-divider></mat-divider>

								<div class="info-row" *ngIf="complaint.assigned_at">
									<span class="label">
										<mat-icon>event</mat-icon>
										Assigned At
									</span>
									<span class="value">
										{{ complaint.assigned_at | date : "medium" }}
									</span>
								</div>
								<div class="info-row">
									<span class="label">
									...
									<span class="value" [class.overdue]="complaint.is_overdue">
										{{ complaint.sla_deadline | date : "medium" }}
										<mat-icon class="overdue-icon" *ngIf="complaint.is_overdue">warning</mat-icon>
									</span>
								</div>
	...
	styles: [
		`
			...
			.sidebar-card mat-card-title mat-icon {
				font-size: 20px;
				width: 20px;
				height: 20px;
...
export class ComplaintDetailComponent implements OnInit {
	...
	get canReopenComplaint(): boolean {
		...
		// Users can reopen within 7 days
		return (
			(this.isOwner || this.isStaffOrAdmin) &&
			this.complaintService.canReopen(this.complaint)
		);
	}
	...
	updateStatus(status: ComplaintStatus): void {
		...
		// Determine confirm color - warn for finalized statuses
		const isFinalStatus =
		...
		const dialogRef = this.dialog.open(ConfirmDialogComponent, {
			data: {
				...
				confirmText: "Update",
				cancelText: "Cancel",
				confirmColor: isFinalStatus ? "warn" : "primary",
			},
		});
				...
				this.complaintService
					...
					.subscribe({
						next: (updated) => {
							...
							this.snackbar.success(`Status updated to ${statusLabel}`);
						},
						error: (err) => {
							const errorMsg = err.error?.error || "Failed to update status";
	...
	assignTo(staffId: string | number): void {
		...
		this.complaintService
			...
			.subscribe({
				...
				error: (err) => {
					const errorMsg = err.error?.error || "Failed to assign complaint";
					this.snackbar.error(errorMsg);
				},
			});
	}
	...
	autoAssign(): void {
		...
		this.complaintService.autoAssignComplaint(this.complaint.id).subscribe({
			...
			error: (err) => {
				const errorMsg = err.error?.error || "Failed to auto-assign complaint";
				this.snackbar.error(errorMsg);
			},
		});
	}
	...
	reopenComplaint(): void {
		const dialogRef = this.dialog.open(ConfirmDialogComponent, {
			data: {
				...
				message:
					"Are you sure you want to reopen this complaint? Please provide a reason.",
				confirmText: "Reopen",
				requireInput: true,
		...
		dialogRef.afterClosed().subscribe((result) => {
			if (result && this.complaint) {
				const reason = result.input || "No reason provided";
				this.complaintService
					.reopenComplaint(this.complaint.id, reason)
					.subscribe({
						next: (updated) => {
							this.complaint = updated;
							this.snackbar.success("Complaint reopened successfully");
						},
					});
			}
		});
