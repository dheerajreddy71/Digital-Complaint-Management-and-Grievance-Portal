      ...
      <ng-container *ngIf="!isLoading && complaint">
        <div class="complaint-header fade-in">
          ...
          <div class="header-content">
            ...
            <div class="meta-info">
              ...
              <span><mat-icon>schedule</mat-icon> {{ complaint.created_at | timeAgo }}</span>
              <span *ngIf="isStaffOrAdmin"><mat-icon>person</mat-icon> {{ complaint.user_name }}</span>
            </div>
          </div>
          ...
          <div class="header-actions" *ngIf="isStaffOrAdmin || isOwner">
            ...
            <button
              mat-icon-button
              color="warn"
              *ngIf="isOwner && complaint.status === 'pending'"
        ...
        <div class="complaint-body">
            ...
            <mat-card class="detail-card fade-in">
              ...
              <mat-card-content>
                ...
                <div class="attachment" *ngIf="complaint.attachment_url">
                  <a [href]="complaint.attachment_url" target="_blank" mat-stroked-button>
          ...
          <div class="sidebar">
            ...
            <!-- Timeline -->
            <app-complaint-timeline [complaintId]="complaint.id"></app-complaint-timeline>
          </div>
        </div>
      </ng-container>
    </div>
...
export class ComplaintDetailComponent implements OnInit {
  ...
  get canChangeStatus(): boolean {
    return (
      this.complaint?.status !== ComplaintStatus.CLOSED &&
      this.complaint?.status !== ComplaintStatus.REJECTED
    );
  }
  ...
  updateStatus(status: ComplaintStatus): void {
    ...
    this.complaintService.updateComplaint(this.complaint.id, { status }).subscribe({
      next: (updated) => {
  ...
  assignTo(staffId: string): void {
    ...
    this.complaintService.assignComplaint(this.complaint.id, staffId).subscribe({
      next: (updated) => {
        this.complaint = updated;
  ...
  autoAssign(): void {
    ...
    this.complaintService.autoAssignComplaint(this.complaint.id).subscribe({
      next: (updated) => {
        this.complaint = updated;
        this.snackbar.success(`Complaint auto-assigned to ${updated.assigned_staff_name}`);
      },
    });
  }
