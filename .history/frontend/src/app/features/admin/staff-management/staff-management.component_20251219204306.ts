...
export class StaffManagementComponent implements OnInit {
	...
	private calculateSummary(): void {
		...
		this.availableStaff = this.staffList.filter((s) => s.availability_status === AvailabilityStatus.AVAILABLE).length;
	...
	toggleAvailability(staff: StaffPerformance & User): void {
		const newAvailability = staff.availability_status !== AvailabilityStatus.AVAILABLE;

		this.userService.updateAvailability(newAvailability, String(staff.id)).subscribe({
			next: () => {
				staff.availability_status = newAvailability ? AvailabilityStatus.AVAILABLE : AvailabilityStatus.OFFLINE;
				this.calculateSummary();
