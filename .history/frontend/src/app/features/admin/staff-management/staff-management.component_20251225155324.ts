			...
			<div class="summary-cards fade-in">
				...
				<mat-card class="summary-card">
					...
					<div class="summary-content">
						...
						<span class="summary-label">Busy (≥3 active)</span>
					</div>
				</mat-card>
				...
				<mat-card class="summary-card">
					...
					<div class="summary-content">
						<span class="summary-value">{{
							avgTeamRating >= 0 ? (avgTeamRating | number : "1.1-1") : "N/A"
						}}</span>
...
export class StaffManagementComponent implements OnInit {
					...
					next: (analytics) => {
						this.staffList = staff.map((s) => {
							...
							const staffPerf: StaffPerformance = {
								...
								in_progress_count: perf?.in_progress_count || 0,
								overdue_count: perf?.overdue_count || 0,
							...
							return {
								...s,
								...staffPerf,
								// Current load = Assigned + In Progress (active_count)
								current_load: perf?.active_count || 0,
								is_available:
	...
	private calculateSummary(): void {
		...
		// Busy = staff with active load >= 3
		this.busyStaff = this.staffList.filter(
			(s) => s.current_load && s.current_load >= 3
		).length;

		const ratings = this.staffList
			.filter((s) => s.avg_rating > 0)
			.map((s) => s.avg_rating);
		// Only show average if there are ratings, otherwise show -1 to indicate N/A
		this.avgTeamRating =
			ratings.length > 0
