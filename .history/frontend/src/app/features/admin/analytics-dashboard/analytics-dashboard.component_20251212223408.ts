		...
		<div class="page-container">
			...
			<ng-container *ngIf="!isLoading && analytics">
				...
				<div class="metrics-grid fade-in">
					...
					<mat-card class="metric-card">
						<mat-card-content>
							...
							<span class="metric-value">{{
								formatDuration(analytics.avg_resolution_time)
							}}</span>
					...
					<mat-card
						class="metric-card"
						[class.warning]="analytics.overdue_count > 0"
					>
						<mat-card-content>
							<div class="metric-header">
							...
							>
								<div class="staff-info">
									...
									<div class="staff-stats">
										...
										<span>•</span>
										<span
											>{{ formatDuration(staff.avg_resolution_time) }} avg</span
										>
										<span>•</span>
										<app-star-rating
								...
								<div class="bar-visual">
									<div
										class="bar-fill"
										[style.width.%]="
											(staff.resolved_count / maxStaffResolved) * 100
										"
									></div>
								</div>
							</div>
...
export class AnalyticsDashboardComponent implements OnInit {
	...
	private processData(data: Analytics): void {
		...
		this.maxStaffResolved = Math.max(
			...data.staff_performance.map((s) => s.resolved_count),
			1
		);
	}
	...
	private calculateResolutionRate(data: Analytics): void {
		const resolved =
			(data.status_breakdown.resolved || 0) +
			(data.status_breakdown.closed || 0);
		this.resolutionRate =
	...
	private processPriorityData(data: Analytics): void {
		...
		this.priorityData = priorities.map((key, i) => ({
			...
			percent: Math.round(
				(((data.priority_breakdown?.[key] as number) || 0) / total) * 100
			),
		}));
	}
