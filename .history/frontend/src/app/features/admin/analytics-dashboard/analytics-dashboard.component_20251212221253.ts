        ...
        <div class="metrics-grid fade-in">
          <mat-card class="metric-card highlight">
            <mat-card-content>
              ...
              <span class="metric-value">{{ analytics.total_complaints }}</span>
          ...
          <mat-card class="metric-card">
            <mat-card-content>
              ...
              <span class="metric-value">{{ formatDuration(analytics.avg_resolution_time) }}</span>
          ...
          <mat-card class="metric-card" [class.warning]="analytics.overdue_count > 0">
            <mat-card-content>
              <div class="metric-header">
                <mat-icon>warning</mat-icon>
              </div>
              <span class="metric-value">{{ analytics.overdue_count }}</span>
              ...
              <div class="donut-chart-container">
                <div class="donut-chart">
                  ...
                  <div class="donut-center">
                    <span class="center-value">{{ analytics.total_complaints }}</span>
              ...
              >
                <div class="staff-info">
                  ...
                  <div class="staff-stats">
                    ...
                    <span>•</span>
                    <span>{{ formatDuration(staff.avg_resolution_time) }} avg</span>
                    <span>•</span>
                    <app-star-rating
                ...
                <div class="bar-visual">
                  <div
                    class="bar-fill"
                    [style.width.%]="(staff.resolved_count / maxStaffResolved) * 100"
                  ></div>
                </div>
              </div>
            </div>
              ...
              <div class="sla-stat">
                <div class="sla-progress">
                  <mat-progress-spinner
                    ...
                    diameter="100"
                    strokeWidth="10"
                    [color]="slaComplianceRate >= 90 ? 'primary' : slaComplianceRate >= 70 ? 'accent' : 'warn'"
                  ></mat-progress-spinner>
              ...
              <div class="sla-breakdown">
                ...
                <div class="sla-item">
                  ...
                  <div class="sla-item-info">
                    ...
                    <span class="sla-item-value">{{ analytics.overdue_count }}</span>
                  </div>
                </div>
              </div>
            </div>
...
export class AnalyticsDashboardComponent implements OnInit {
  ...
  statusData: Array<{ key: string; label: string; count: number; percent: number; color: string }> = [];
  statusSegments: Array<{ color: string; dashArray: string; dashOffset: number }> = [];
  categoryData: Array<{ key: string; label: string; count: number; color: string }> = [];
  maxCategoryCount = 1;
  priorityData: Array<{ key: string; label: string; count: number; percent: number }> = [];
  ...
  loadAnalytics(): void {
    ...
    this.analyticsService.getAnalytics({ period: this.selectedPeriod }).subscribe({
      next: (data) => {
        this.analytics = data;
  ...
  private processData(data: Analytics): void {
    ...
    this.maxStaffResolved = Math.max(...data.staff_performance.map((s) => s.resolved_count), 1);
  }

  private calculateResolutionRate(data: Analytics): void {
    const resolved = (data.status_breakdown.resolved || 0) + (data.status_breakdown.closed || 0);
    this.resolutionRate = data.total_complaints > 0
      ? Math.round((resolved / data.total_complaints) * 100)
      : 0;
  }

  private processStatusData(data: Analytics): void {
    const total = data.total_complaints || 1;
    this.statusData = Object.entries(data.status_breakdown).map(([key, count]) => ({
      key,
      label: this.statusLabels[key] || key,
      count: count as number,
      percent: Math.round(((count as number) / total) * 100),
      color: this.statusColors[key] || '#9e9e9e',
    }));
  ...
  private processCategoryData(data: Analytics): void {
    this.categoryData = Object.entries(data.category_breakdown).map(([key, count]) => ({
      key,
      label: this.categoryLabels[key] || key,
      count: count as number,
      color: this.categoryColors[key] || '#607d8b',
    }));
    this.categoryData.sort((a, b) => b.count - a.count);
    this.maxCategoryCount = Math.max(...this.categoryData.map((c) => c.count), 1);
  }
  ...
  private processPriorityData(data: Analytics): void {
    ...
    this.priorityData = priorities.map((key, i) => ({
      key,
      label: labels[i],
      count: (data.priority_breakdown?.[key] as number) || 0,
      percent: Math.round((((data.priority_breakdown?.[key] as number) || 0) / total) * 100),
    }));
  }
  ...
  private calculateSlaStats(data: Analytics): void {
    ...
    this.slaComplianceRate = resolved > 0 ? Math.round((this.onTimeCount / resolved) * 100) : 100;
  }
