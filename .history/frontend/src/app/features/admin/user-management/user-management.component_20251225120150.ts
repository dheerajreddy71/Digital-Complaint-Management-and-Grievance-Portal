				...
				<div class="table-container">
					<table mat-table [dataSource]="dataSource" matSort>
						...
						<ng-container matColumnDef="department">
							...
							<td mat-cell *matCellDef="let user">
								<span *ngIf="user.department" class="department-tag">
									{{ user.department }}
								</span>
								<span *ngIf="!user.department" class="text-muted">-</span>
							</td>
						</ng-container>
				...
				<mat-dialog-content>
					<form [formGroup]="userForm" class="user-form">
						...
						<mat-form-field
							appearance="outline"
							class="full-width"
							*ngIf="userForm.get('role')?.value === 'staff'"
						>
							<mat-label>Department</mat-label>
							<mat-select formControlName="department">
								<mat-option value="Plumbing">Plumbing</mat-option>
								<mat-option value="Electrical">Electrical</mat-option>
								<mat-option value="Facility Management"
									>Facility Management</mat-option
								>
								<mat-option value="IT Support">IT Support</mat-option>
								<mat-option value="General Administration"
									>General Administration</mat-option
								>
							</mat-select>
						</mat-form-field>
					</form>
...
export class UserManagementComponent implements OnInit {
	...
	filterByRole(): void {
		if (this.roleFilter) {
			this.dataSource.data = this.allUsers.filter(
				(u) => u.role.toLowerCase() === this.roleFilter.toLowerCase()
			);
		} else {
	...
	saveUser(): void {
		...
		if (this.isEditMode && this.editingUser) {
			...
			this.userService.updateUser(String(this.editingUser.id), data).subscribe({
				next: (updated) => {
	...
	changeRole(user: User): void {
		...
		dialogRef.afterClosed().subscribe((confirmed) => {
			if (confirmed) {
				this.userService
					.updateUser(String(user.id), { role: nextRole })
					.subscribe({
						next: (updated) => {
							const index = this.allUsers.findIndex((u) => u.id === updated.id);
							if (index !== -1) {
