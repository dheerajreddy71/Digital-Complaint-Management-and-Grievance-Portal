					...
					<mat-card-content>
						<div class="table-container">
							...
							>
								...
								<td mat-cell *matCellDef="let staff">{{ staff.staff_name }}</td>
							</ng-container>
							...
							{{ formatDuration(staff.average_resolution_hours) }}
								<ng-container matColumnDef="rating">
									...
									size="small"
								></app-star-rating>
								<span>{{ staff.average_rating | number : "1.1-1" }}</span>
...
export class AdminOverviewComponent implements OnInit {
	...
	staffColumns = ["name", "resolved", "avgTime", "rating"];
	...
	getStatusCount(status: string): number {
		...
		const item = this.analytics.by_status.find((s) => s.status === status);
		return item?.count || 0;
	}

	getStatusPercentage(count: number): number {
		if (!this.analytics?.overview.total_complaints) return 0;
		return (count / this.analytics.overview.total_complaints) * 100;
	}
