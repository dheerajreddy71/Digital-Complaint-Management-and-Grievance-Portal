          ...
          <div class="complaints-list" *ngIf="!isLoading && complaints.length > 0">
            ...
            >
              <div class="complaint-main">
                ...
                <p class="complaint-desc">{{ complaint.description | truncate: 100 }}</p>
...
export class UserDashboardComponent implements OnInit {
  ...
  private loadComplaints(): void {
    this.complaintService.getMyComplaints({ limit: 5 }).subscribe({
      next: (response) => {
  ...
  private calculateStats(complaints: Complaint[]): void {
    this.stats = {
      pending: complaints.filter(
        (c) => c.status === ComplaintStatus.PENDING || c.status === ComplaintStatus.ASSIGNED
      ).length,
