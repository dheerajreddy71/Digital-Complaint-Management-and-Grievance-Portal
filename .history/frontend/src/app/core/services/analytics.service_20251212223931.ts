...
import { AnalyticsData, StaffPerformance, AuditLog, PaginatedResponse, Feedback } from "../models";
...
export class AnalyticsService {
	...
	getAnalytics(
		filters?: { period?: string; start_date?: string; end_date?: string } | string
	...
	getStaffPerformance(staffId: string): Observable<StaffPerformance> {
		return this.http.get<StaffPerformance>(`${this.API_URL}/staff/${staffId}/performance`);
	}

	getStaffFeedback(staffId: string): Observable<Feedback[]> {
		return this.http.get<Feedback[]>(`${this.API_URL}/staff/${staffId}/feedback`);
	}
	...
	getAuditLogs(params: any): Observable<PaginatedResponse<AuditLog>> {
		...
		return this.http.get<PaginatedResponse<AuditLog>>(`${environment.apiUrl}/audit-logs`, { params: httpParams });
	}
}
