...
export class ComplaintService {
	...
	reopenComplaint(id: string, reason: string): Observable<Complaint> {
		return this.http.post<Complaint>(`${this.API_URL}/${id}/reopen`, {
			reason,
		});
	}

	canReopen(complaint: Complaint): boolean {
		...
		const daysSinceResolved =
			(new Date().getTime() - resolvedDate.getTime()) / (1000 * 60 * 60 * 24);
