...
export class UserService {
	...
	updateProfile(updates: { name?: string; email?: string; phone?: string }): Observable<User> {
		return this.http.put<User>(`${this.API_URL}/me`, updates);
	}

	updateAvailability(status: AvailabilityStatus | boolean, userId?: string): Observable<User> {
		const availabilityStatus = typeof status === 'boolean' 
			? (status ? AvailabilityStatus.AVAILABLE : AvailabilityStatus.OFFLINE)
			: status;
		const url = userId ? `${this.API_URL}/${userId}/availability` : `${this.API_URL}/me/availability`;
