...
export const startEscalationJob = () => {
	cron.schedule("0 * * * *", async () => {
		try {
			...
			const result = await complaintRepo.findAll({
				status: ["Open", "Assigned", "In-progress"] as ComplaintStatus[],
			});

			const activeComplaints = result.data;
			...
			for (const complaint of activeComplaints) {
				...
				if (shouldEscalate) {
					...
					logger.info(
						`Escalated complaint #${complaint.id}: ${escalationReason}`
					);
					escalatedCount++;
				}
			}
