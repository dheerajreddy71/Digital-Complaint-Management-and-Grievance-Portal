...
export class CommentService {
	...
	async create(
		...
		if (complaint.assigned_to && complaint.assigned_to !== userId) {
			notifyUsers.add(complaint.assigned_to);
		}
	...
	// Get all comments for a complaint (filter internal comments for non-staff)
	async getByComplaintId(
		complaintId: number,
		userRole?: string
	): Promise<CommentResponse[]> {
		const comments = await this.commentRepo.findByComplaintId(complaintId);

		// If user is not Staff or Admin, filter out internal comments
