...
export class ComplaintService {
	...
	async create(
		...
		const isRecurring = await this.complaintRepo.checkRecurring(
			data.category,
			data.location
		);
		if (isRecurring) {
	...
	// Staff pickup with race condition protection
	async pickup(
		complaintId: number,
		staffId: number,
		staffDepartment: string | null,
		ipAddress?: string
	): Promise<{ success: boolean; complaint?: ComplaintResponse; error?: string }> {
		const result = await this.complaintRepo.pickupWithTransaction(
			complaintId,
			staffId,
			staffDepartment
		);
		...
		await this.statusRepo.create({
			...
			new_status: "Assigned",
			updated_by: staffId,
			notes: `Picked up by staff #${staffId}`,
		});

		// Notify user that their complaint was picked up
		await this.notificationRepo.create({
			user_id: complaint.user_id,
			complaint_id: complaintId,
			type: "Assigned",
			message: `Your complaint "${complaint.title}" has been picked up by a technician`,
		});

		// Audit log
		await this.auditRepo.create({
			user_id: staffId,
			action: "COMPLAINT_PICKED_UP",
			entity_type: "Complaint",
			entity_id: complaintId,
			details: { picked_by: staffId, department: staffDepartment },
			ip_address: ipAddress,
		});

		logger.info(`Complaint #${complaintId} picked up by staff #${staffId}`);
	...
	// Update complaint status with proper workflow validation
	async updateStatus(
		complaintId: number,
		...
		// Update complaint with new status
		const updateData: UpdateComplaintDto = { status };

		// If resolving, set resolved_at timestamp
		if (status === "Resolved") {
			// Resolution notes should be stored (if repository supports it)
			// For now, we'll add it to the status history notes
		}

		const updated = await this.complaintRepo.update(complaintId, updateData);

		// Notify user about status change
		await this.notificationRepo.create({
			user_id: current.user_id,
			complaint_id: complaintId,
			type: status === "Resolved" ? "Resolved" : "StatusUpdate",
			message: `Your complaint "${current.title}" status changed to ${status}`,
		});
		...
		// Notify assigned staff if status changed by someone else
		if (current.assigned_to && current.assigned_to !== updatedBy) {
			await this.notificationRepo.create({
				user_id: current.assigned_to,
				complaint_id: complaintId,
				type: "StatusUpdate",
				message: `Status updated for "${current.title}" to ${status}`,
			});
		}
		...
		await this.auditRepo.create({
			...
			entity_type: "Complaint",
			entity_id: complaintId,
			details: { old_status: current.status, new_status: status, notes },
			ip_address: ipAddress,
		});

		logger.info(
			`Complaint #${complaintId} status updated: ${current.status} → ${status}`
		);

		return updated;
	}

	// Reopen resolved/closed complaint
	async reopen(
		complaintId: number,
		reason: string,
		requestedBy: number,
		...
		const updated = await this.complaintRepo.update(complaintId, {
			status: "Open",
		});

		// Mark as recurring if reopened
		await this.complaintRepo.markAsRecurring(complaintId);

		// Notify previously assigned staff
		...
		for (const admin of admins.filter((u) => u.role === "Admin")) {
			await this.notificationRepo.create({
				...
				complaint_id: complaintId,
				type: "Reminder",
				message: `Alert: Complaint #${complaintId} reopened - may require attention`,
			});
		}
		...
		await this.auditRepo.create({
			...
			entity_type: "Complaint",
			entity_id: complaintId,
			details: { reason, previous_status: complaint.status },
			ip_address: ipAddress,
		});

		logger.warn(`Complaint #${complaintId} reopened by user #${requestedBy}`);
	...
	// Validate status transition follows: Open → Assigned → In-progress → Resolved → Closed
	private validateStatusTransition(
		current: ComplaintStatus,
		next: ComplaintStatus
	): void {
		const validTransitions: Record<ComplaintStatus, ComplaintStatus[]> = {
			Open: ["Assigned", "In-progress", "Closed"], // Can skip to In-progress or close if invalid
			Assigned: ["In-progress", "Open", "Resolved", "Closed"], // Can go back or skip
			"In-progress": ["Resolved", "Assigned", "Closed"], // Can reassign or resolve
			Resolved: ["Closed", "Open"], // Can close or reopen (within window)
			Closed: ["Open"], // Can reopen (admin only)
		};
