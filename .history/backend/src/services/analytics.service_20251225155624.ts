...
export class AnalyticsService {
	...
	async getAnalytics(dateFrom?: Date, dateTo?: Date): Promise<AnalyticsData> {
       ...
       FROM complaints WHERE 1=1 ${dateFilter}`
		...
		// SLA compliance rate - considers both resolved and active overdue complaints
		// Good: resolved within SLA or active complaints not yet overdue
		// Bad: resolved late OR currently overdue (regardless of status)
		const slaRow = await query<RowDataPacket[]>(
			`SELECT 
        ROUND(
          (SELECT COUNT(*) FROM complaints WHERE status = 'Resolved' AND is_overdue = FALSE ${dateFilter}) * 100.0 / 
          NULLIF(
            (SELECT COUNT(*) FROM complaints WHERE status = 'Resolved' ${dateFilter}) + 
            (SELECT COUNT(*) FROM complaints WHERE is_overdue = TRUE AND status != 'Resolved' ${dateFilter}),
            0
          ), 1
        ) as rate
       FROM dual`
       ...
       ORDER BY date ASC`
        ...
        COUNT(CASE WHEN c.assigned_to = u.id AND c.status IN ('Assigned', 'In-progress') THEN 1 END) as active_count,
