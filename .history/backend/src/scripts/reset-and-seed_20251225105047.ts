...
async function main() {
	...
	try {
		...
		for (const template of complaintTemplates) {
          ...
          user_id, staff_id, title, description, category, priority, 
          location, status, sla_deadline, is_overdue, resolved_at, created_at
			...
			await connection.query(
				`INSERT INTO status_history (complaint_id, changed_by, from_status, to_status, created_at)
         VALUES (?, ?, 'None', 'Open', ?)`,
			...
			await connection.query(
				`INSERT INTO status_history (complaint_id, changed_by, from_status, to_status, notes, created_at)
         ...
         VALUES (?, ?, 'Assigned', 'Assigned', ?, ?)`,
           ...
           VALUES (?, ?, 'Assigned', 'In-progress', ?, ?)`,
             ...
             VALUES (?, ?, 'In-progress', 'Resolved', ?, ?)`,
					...
					await connection.query(
						`INSERT INTO feedback (complaint_id, user_id, rating, comments, is_resolved, created_at)
             VALUES (?, ?, ?, ?, TRUE, ?)`,
						[
							complaintId,
							userId,
