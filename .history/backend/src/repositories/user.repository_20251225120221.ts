...
export class UserRepository {
	...
	async findById(id: number): Promise<User | null> {
		const rows = await query<RowDataPacket[]>(
			`SELECT id, name, email, password, role, department, 
             LOWER(availability_status) as availability_status, 
             contact_info, expertise, is_active, created_at, updated_at
       FROM users 
       WHERE id = ?`,
			[id]
		);
	...
	async create(data: CreateUserDto): Promise<UserResponse> {
		...
		const result = await query<ResultSetHeader>(
			`INSERT INTO users (name, email, password, role, department, contact_info, expertise)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
			[
				data.name,
				data.email,
	...
	async findAllStaff(): Promise<UserResponse[]> {
		const rows = await query<RowDataPacket[]>(
			`SELECT id, name, email, role, department, 
             LOWER(availability_status) as availability_status, 
             contact_info, expertise, is_active, created_at
       FROM users 
	...
	> {
		const rows = await query<RowDataPacket[]>(
			`SELECT u.id, u.name, u.email, u.role, u.department,
              LOWER(u.availability_status) as availability_status,
              u.contact_info, u.expertise, u.is_active, u.created_at,
              COUNT(c.id) as active_complaints
       FROM users u
       LEFT JOIN complaints c ON u.id = c.assigned_to AND c.status != 'Resolved'
       WHERE u.role = 'Staff' AND u.is_active = TRUE AND LOWER(u.availability_status) = 'available'
       GROUP BY u.id
