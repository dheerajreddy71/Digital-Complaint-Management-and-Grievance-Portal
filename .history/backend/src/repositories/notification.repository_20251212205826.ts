...
import { Notification, CreateNotificationDto, NotificationResponse } from '../models';
...
export class NotificationRepository {
  ...
  async findByUserId(userId: number, limit: number = 50): Promise<NotificationResponse[]> {
    const rows = await query<RowDataPacket[]>(
  ...
  async createBatch(notifications: CreateNotificationDto[]): Promise<void> {
    ...
    const values = notifications.map(n => 
      [n.user_id, n.complaint_id || null, n.type, n.message]
    );
