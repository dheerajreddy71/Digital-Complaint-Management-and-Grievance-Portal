...
import { StatusHistory, CreateStatusHistoryDto, StatusHistoryResponse } from '../models';
...
export class StatusHistoryRepository {
  ...
  async create(data: CreateStatusHistoryDto): Promise<StatusHistory> {
    const result = await query<ResultSetHeader>(
      `INSERT INTO status_history (complaint_id, previous_status, new_status, updated_by, notes)
       VALUES (?, ?, ?, ?, ?)`,
      [data.complaint_id, data.previous_status, data.new_status, data.updated_by, data.notes || null]
    );
  ...
  async getComplaintTimeline(complaintId: number): Promise<StatusHistoryResponse[]> {
    const rows = await query<RowDataPacket[]>(
