...
export class ComplaintRepository {
	...
	async findAll(
		...
		// Staff-specific: show complaints from category OR assigned to staff
		if (filters.staff_category_or_assigned) {
			whereConditions.push("(c.category = ? OR c.assigned_to = ?)");
			params.push(filters.staff_category_or_assigned.category, filters.staff_category_or_assigned.staff_id);
		}
	...
	// Assign staff to complaint with race condition protection
	async assign(
		complaintId: number,
		staffId: number
	): Promise<ComplaintResponse | null> {
	...
	// Transaction-safe pickup for staff (prevents race conditions)
	async pickupWithTransaction(
		complaintId: number,
		staffId: number,
		...
		try {
			...
			const [rows] = await connection.execute<RowDataPacket[]>(
				`SELECT c.*, u.department as staff_department 
				 FROM complaints c
			...
			if (complaint.assigned_to !== null) {
				await connection.rollback();
				return { success: false, error: "This complaint was already picked up by another staff member" };
			}
			
			// Verify department matches (staff can only pick from their department)
			...
			if (staffDepartment && expectedDept && staffDepartment !== expectedDept) {
				await connection.rollback();
				return { success: false, error: "You don't have access to complaints from this department" };
			}
