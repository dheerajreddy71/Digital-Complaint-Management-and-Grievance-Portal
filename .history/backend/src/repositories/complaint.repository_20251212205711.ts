...
export class ComplaintRepository {
  ...
  async create(userId: number, data: CreateComplaintDto): Promise<ComplaintResponse> {
    ...
    const result = await query<ResultSetHeader>(
      `INSERT INTO complaints 
       ...
       VALUES (?, ?, ?, ?, ?, ?, 'Open', ?, ?)`,
      [userId, data.title, data.description, data.category, data.priority, data.location, slaDeadline, attachments]
    );
  ...
  async findAll(filters: ComplaintFilters): Promise<PaginatedResponse<ComplaintResponse>> {
    ...
    if (filters.search) {
      whereConditions.push('(c.title LIKE ? OR c.description LIKE ? OR c.location LIKE ?)');
  ...
  // Update complaint
  async update(id: number, data: UpdateComplaintDto): Promise<ComplaintResponse | null> {
    const fields: string[] = [];
    ...
    values.push(id);
    await query(
      `UPDATE complaints SET ${fields.join(', ')}, updated_at = NOW() WHERE id = ?`,
      values
    );
  ...
  async assign(complaintId: number, staffId: number): Promise<ComplaintResponse | null> {
    await query(
      `UPDATE complaints 
       SET staff_id = ?, status = 'Assigned', updated_at = NOW() 
       WHERE id = ?`,
      [staffId, complaintId]
    );
