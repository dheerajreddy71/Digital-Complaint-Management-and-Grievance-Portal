...
export class ComplaintRepository {
	...
	async findAll(
		...
		if (filters.staff_category_or_assigned) {
			...
			params.push(
				filters.staff_category_or_assigned.category,
				filters.staff_category_or_assigned.staff_id
			);
		}
	...
	async pickupWithTransaction(
		...
		try {
			...
			if (complaint.assigned_to !== null) {
				...
				return {
					success: false,
					error: "This complaint was already picked up by another staff member",
				};
			}
			...
			if (staffDepartment && expectedDept && staffDepartment !== expectedDept) {
				...
				return {
					success: false,
					error: "You don't have access to complaints from this department",
				};
			}
	...
	private toResponse(row: any): ComplaintResponse {
		return {
			...
			assigned_staff_department: row.assigned_staff_department || null,
