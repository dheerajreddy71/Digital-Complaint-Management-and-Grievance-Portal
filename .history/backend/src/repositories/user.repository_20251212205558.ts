...
export class UserRepository {
  ...
  async create(data: CreateUserDto): Promise<UserResponse> {
    ...
    const result = await query<ResultSetHeader>(
      `INSERT INTO users (name, email, password, role, contact_info, expertise)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [data.name, data.email, hashedPassword, data.role, data.contact_info || null, data.expertise || null]
    );
  ...
  async getStaffWithWorkload(): Promise<(UserResponse & { active_complaints: number })[]> {
    const rows = await query<RowDataPacket[]>(
      `SELECT u.*, 
              COUNT(c.id) as active_complaints
       FROM users u
       LEFT JOIN complaints c ON u.id = c.staff_id AND c.status != 'Resolved'
  ...
  async updateAvailability(userId: number, status: 'Available' | 'OnLeave' | 'Busy'): Promise<void> {
    await query(
  ...
  async update(id: number, data: Partial<User>): Promise<UserResponse | null> {
    ...
    if (data.contact_info) { fields.push('contact_info = ?'); values.push(data.contact_info); }
    if (data.expertise) { fields.push('expertise = ?'); values.push(data.expertise); }
    if (data.availability_status) { fields.push('availability_status = ?'); values.push(data.availability_status); }
  ...
  async findAll(): Promise<UserResponse[]> {
    const rows = await query<RowDataPacket[]>('SELECT * FROM users ORDER BY created_at DESC');
    return rows.map(row => this.toResponse(row as User));
  }
  ...
  private toResponse(user: User): UserResponse {
    const { password, updated_at, ...response } = user;
    return response;
  }
}
