...
import { authenticate, authorize, complaintRateLimiter, uploadMultiple } from '../middlewares';
...
router.post('/check-duplicates', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const result = await complaintService.checkDuplicates(req.body, req.user!.userId);
    res.json(result);
  } catch (error) {
    next(error);
  }
...
router.get('/:id/timeline', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const timeline = await complaintService.getTimeline(parseInt(req.params.id));
    res.json(timeline);
  } catch (error) {
    next(error);
  }
    ...
    try {
      ...
      if (req.user!.role === 'User') {
        ...
        if (complaint.status !== 'Open') {
          res.status(400).json({ error: 'Cannot modify complaint after it has been assigned' });
          return;
        }
      }
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await complaintService.bulkAssign(complaint_ids, staff_id, req.user!.userId);
...
router.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await complaintService.bulkUpdateStatus(complaint_ids, status, req.user!.userId);
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      await complaintService.delete(parseInt(req.params.id), req.user!.userId, req.ip);
      res.status(204).send();
    } catch (error) {
      next(error);
    }
