		...
		try {
			...
			if (req.user!.role === "User") {
				...
				if (req.query.assigned_to === "null") {
					...
					// Show only complaints assigned to this staff
				...
				if (req.query.all && staffCategory) {
					// Use OR filter: category matches OR assigned to this staff
					filters.staff_category_or_assigned = {
						category: staffCategory,
						staff_id: req.user!.userId
					};
				}
			} else if (req.user!.role === "Admin") {
				// Admin can filter by assigned_to=me to see their own assigned complaints
				if (req.query.assigned_to === "me") {
					filters.assigned_to = req.user!.userId;
				}
			}
			// Admin sees all (when no specific filter)
...
const canStaffAccessComplaint = (
	...
	// Staff can access if:
	// 1. Complaint is assigned to them
	if (complaint.assigned_to === staffId) return true;
	
	// 2. Complaint is unassigned AND belongs to their department
	...
	// 3. Otherwise, no access (assigned to another staff)
	return false;
};
		...
		try {
			...
			if (req.user!.role === "User") {
				...
				if (complaint.user_id !== req.user!.userId) {
					res.status(403).json({ error: "You can only view your own complaints" });
					return;
				}
			} else if (req.user!.role === "Staff") {
				// Staff can only view:
				// - Complaints assigned to them
				// - Unassigned complaints from their department
				if (!canStaffAccessComplaint(req.user!.userId, req.user!.department, complaint)) {
					res.status(403).json({ error: "You don't have access to this complaint" });
					return;
				}
			}
...
router.get(
	...
	async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			const timeline = await complaintService.getTimeline(complaintId);
			res.json(timeline);
		} catch (error) {
			next(error);
		}
...
router.post(
	...
	async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			// Staff can only self-assign (pick up) using transaction-safe method
			if (req.user!.role === "Staff") {
				const result = await complaintService.pickup(
					complaintId,
					req.user!.userId,
				...
				if (!result.success) {
					const statusCode = result.error?.includes("not found") ? 404 : 
									   result.error?.includes("already picked") ? 409 :
									   result.error?.includes("don't have access") ? 403 : 400;
