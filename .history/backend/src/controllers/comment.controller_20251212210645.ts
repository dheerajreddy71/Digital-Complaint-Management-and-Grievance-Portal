...
router.get('/complaint/:id', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const comments = await commentService.getByComplaintId(parseInt(req.params.id));
    res.json(comments);
  } catch (error) {
    next(error);
  }
...
router.delete('/:id', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    const deleted = await commentService.delete(parseInt(req.params.id), req.user!.userId);
    if (!deleted) {
