	...
	async (req: Request, res: Response, next: NextFunction) => {
		try {
			...
			if (req.user!.role === "User") {
				...
				// Staff can see unassigned (queue) or their assigned complaints
				if (req.query.assigned_to !== "null" && !req.query.all) {
					filters.assigned_to = req.user!.userId;
				} else if (req.query.assigned_to === "null") {
					// Allow staff to see queue (unassigned complaints)
