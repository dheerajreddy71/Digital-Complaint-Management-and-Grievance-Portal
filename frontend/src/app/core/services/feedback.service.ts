...
export class FeedbackService {
	...
	getFeedbackByComplaint(complaintId: string): Observable<Feedback | null> {
		return this.http.get<Feedback | null>(
			`${this.API_URL}/complaint/${complaintId}`
		);
	}

	getFeedbackByComplaintId(complaintId: string): Observable<Feedback | null> {
		return this.getFeedbackByComplaint(complaintId);
	}
	...
	getStaffPerformance(
		...
		return this.http.get<StaffPerformanceMetrics[]>(
			`${this.API_URL}/staff-performance`,
			{
				params,
			}
		);
	...
	getAllFeedback(
		page = 1,
		limit = 10
	): Observable<{ data: Feedback[]; total: number }> {
		const params = new HttpParams()
			.set("page", String(page))
			.set("limit", String(limit));

		return this.http.get<{ data: Feedback[]; total: number }>(this.API_URL, {
			params,
		});
	}
}
