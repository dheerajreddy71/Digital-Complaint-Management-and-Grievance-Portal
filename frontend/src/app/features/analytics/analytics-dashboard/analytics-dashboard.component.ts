				...
				<div class="stats-grid fade-in">
					...
					<mat-card class="stat-card">
						...
						<div class="stat-content">
							<span class="stat-value"
								>{{
									analytics.overview.sla_compliance_rate | number : "1.0-0"
								}}%</span
							>
						...
						<div class="locations-list">
							...
							>
								...
								<span class="location-count"
									>{{ location.count }} complaints</span
								>
							</div>
						</div>
					</mat-card-content>
...
export class AnalyticsDashboardComponent implements OnInit {
	...
	loadAnalytics(): void {
		...
		const filters: any = {};
		if (this.dateFrom) {
			filters.start_date = this.dateFrom.toISOString().split("T")[0];
		}
		if (this.dateTo) {
			filters.end_date = this.dateTo.toISOString().split("T")[0];
		}

		this.analyticsService
			.getAnalytics(Object.keys(filters).length > 0 ? filters : undefined)
			.subscribe({
				next: (data) => {
					this.analytics = data;
					this.setupCharts();
	...
	private setupCharts(): void {
		...
		this.statusChartOptions = {
			series: this.analytics.by_status.map((s: any) => s.count),
			chart: {
				type: "pie",
				height: 350,
			},
			labels: this.analytics.by_status.map((s: any) => s.status),
		...
		this.categoryChartOptions = {
			...
			xaxis: {
				categories: this.analytics.by_category.map((c: any) => c.category),
			},
			yaxis: {
				title: {
		...
		this.priorityChartOptions = {
			series: this.analytics.by_priority.map((p: any) => p.count),
			chart: {
				type: "donut",
				height: 350,
			},
			labels: this.analytics.by_priority.map((p: any) => p.priority),
		...
		this.trendChartOptions = {
			...
			xaxis: {
				categories: this.analytics.trends.map((t: any) => t.date),
				labels: {
